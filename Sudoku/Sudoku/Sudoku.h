...
class Sudoku {
 private:
	...
	int grid_num_ = 0;  // 棋盘数目
 public:
	void RandomLine(int line[]);
	bool JudgeLegitimacy(SudokuGrids* sudoku_grid,
		int row, int col, int val);  // 创建时这个判断更快
	bool JudgeLegitimacy1(SudokuGrids* sudoku_grid,
		int row, int col, int val);  // 求解时再用这个判断
	...
	void BackTrace(SudokuGrids* sudoku_grid,
		int n);  // 回溯法解数独 n = row * 9 + col，只找出一个解
